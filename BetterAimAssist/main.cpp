...
void trackThemDown() {
    ...
    while (true) {
        ...
        /*
        // Optional capture saving
        if (isTargetFound) {
            char fileName[20];
            sprintf_s(fileName, sizeof(fileName), "capture_%d.bmp", i++); // i is a global defined counter
            pixelsToBMP(fileName, screenshotPixels, config.scanAreaX, config.scanAreaY);
        }
        */
...
int main() {
    startupRoutine(); // Execute startup routine (checking config and typeshi)
